...
class StripeWH_Handler:
    ...
    def handle_payment_intent_succeeded(self, event):
        ...
        if order_exists:
            return HttpResponse(
                    content=f"Webhook recived: {event['type']} | SUCCESS: Verified order already exists",
                    status=200)
        else:
            order = None
            try:
                order = Object.objects.create(
                ...
                if order:
                    order.delete()
                return HttpResponse(
                    content=f"Webhook recived: {event['type']} | ERROR: {e}", status=500)
